...
import * as api from './services/firebaseService'; // Import our new service
...
const TestScreen: React.FC<TestScreenProps> = ({ level, onComplete, levelParamsInt, levelParamsFrac }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-lg shadow-lg w-full text-center">
            <p className="text-4xl font-mono tracking-wider mb-6">{currentQuestion.text}</p>
            {currentQuestion.type === 'integer' ? (
                <input 
                    ...
                    className="w-48 text-center text-3xl p-2 bg-slate-200 dark:bg-slate-700 rounded-md border-2 border-transparent focus:border-blue-500 focus:outline-none"
                    autoFocus
                />
            ) : (
                <div className="inline-flex flex-col items-center">
                    <input 
                        ...
                        autoFocus
                    />
                    <div className="w-36 h-1 bg-slate-900 dark:bg-slate-200 my-2"></div>
                    <input 
                        ref={denRef}
                        type="text" 
                        value={denAnswer}
        ...
        <div className="w-full md:flex md:justify-between md:items-start gap-4">
          <div className="flex-grow"></div>
          <NumberPad onKeyPress={handleKeyPress} />
          <div className="flex-grow flex justify-end items-end mt-4 md:mt-0">
             <button onClick={handleNextQuestion} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg text-lg transition-transform transform hover:scale-105">
