...
const multiplyInteger = (params: number[]): Omit<Question, 'id'> => {
    ...
    const answer = n1 * n2;
    return { text: `${n1} × ${n2} = ?`, type: 'integer', answer, operationType: 'mul', operands: [n1, n2] };
};
...
const divideInteger = (level: number, params: number[]): Omit<Question, 'id'> => {
    ...
    return { text: `${n1} ÷ ${n2} = ?`, type: 'integer', answer, operationType: 'div', operands: [n1, n2] };
};
...
const addFraction = (params: number[]): Omit<Question, 'id'> => {
    ...
    return { text: `${r1.num}/${r1.den} + ${r2.num}/${r2.den} = ?`, type: 'rational', answer, operationType: 'add', operands: [r1, r2] };
};
...
const subtractFraction = (params: number[]): Omit<Question, 'id'> => {
    ...
    const result = { num: r1.num * r2.den - r2.num * r1.den, den: r1.den * r2.den };
    const answer = simplifyFraction(result);
    return { text: `${r1.num}/${r1.den} - ${r2.num}/${r2.den} = ?`, type: 'rational', answer, operationType: 'sub', operands: [r1, r2] };
};
...
const multiplyFraction = (params: number[]): Omit<Question, 'id'> => {
    ...
    const answer = simplifyFraction(result);
    return { text: `${r1.num}/${r1.den} × ${r2.num}/${r2.den} = ?`, type: 'rational', answer, operationType: 'mul', operands: [r1, r2] };
};
...
const divideFraction = (params: number[]): Omit<Question, 'id'> => {
    ...
    const answer = simplifyFraction(result);
    return { text: `${r1.num}/${r1.den} ÷ ${r2.num}/${r2.den} = ?`, type: 'rational', answer, operationType: 'div', operands: [r1, r2] };
};
